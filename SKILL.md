...
# 帧率优化助手 · 一键画面优化（通用 Agent 技能）

> **非官方个人项目，与腾讯公司及《三角洲行动》官方无任何关系。** 本技能只修改 Windows 系统层设置，不读写游戏进程、不注入、不与反作弊交互，也不修改游戏内设置。本文可能被远程单独读取，因此这句话写在最前面。
...
旧版 v2 备份没有可靠项目归属，只支持全部复原；**这份备份自己的改动全部入账后**整份标为
`.restored`（同一次还原里别的备份失败不会扣住它）。

记账是按单条改动走的：只有「确认写回了原值」和「执行了安全回退且重试也不可能成功」两类
才会被消费。失败的、以及这次没成但下次可能成的，都留着可重试——所以一条失败不会连累
其余已经还原好的改动，重试时也只会重放真正失败的那几条。

结果字段：

- `RestoredOps`——实际写回原值的条数。
- `Failed`——真失败（带人话项名）。对应改动仍留在系统里，备份保留，排查后可重试。
- `Skipped`——**执行了，但没回到原来的值**。包含各类安全回退：原电源方案已被删除而退到
  Windows「平衡」、电源隐藏项精确清理被策略拦住而写回方案默认值或切走活动方案、备份里
  根本没记录原电源方案。**这些都有实际影响**，转述时必须如实告诉用户哪些项目没有回到
  原样、以及需要他手动做什么；不要说成「不影响任何生效设置」。
- `BookkeepingFailed`——系统设置**已经**写回去了，但这次还原没记上账（凭证写入或 `.restored`
  改名失败）。这不是还原失败，别说成「改动仍留在系统中」。真正的风险是下次还原会把同样的
  旧值再写一遍，覆盖用户此后的手动修改——先让用户修好备份目录，不要直接重试。
- `RestoredItemIds`——**全部**改动都写回了原值的项目；只还原了一半的项目不会出现在这里。
...
```
powershell -NoProfile -ExecutionPolicy Bypass -File "<root>\scripts\delta-booster.ps1" -ListSymptoms [-Json]
```

优化项名写的全是**手段**（`HwSchMode`、MMCSS、`DisablePagingExecutive`），用户找的是
**症状**。30 多项摊在一页里，真正需要某一项的人在列表里认不出它来。`Get-SymptomCatalog`
把 19 条症状映射到优化项，图形界面把它画成「优化项」列表上方的筛选带。

这 19 个症状 Id 与诊断报告里的「当前问题」多选项是**同一份数据**，只有一个来源。

三条刻意的取舍：

| 取舍 | 为什么 |
|---|---|
| 映射不到优化项就如实说「没有」，并指向该去的地方（掉帧修复页 / 游戏内设置参考 / 复原入口 / 诊断报告 / 残留反查） | 给一条本工具治不了的症状硬凑几个项目，比承认治不了更坏：用户照着执行一遍没好，剩下的结论他也不会再信 |
| `cpu_heat` / `gpu_heat` / `noise_power` 三条的 `Items` **刻意为空**，一律指向复原入口 | 本工具没有降温项，而「卓越性能」「锁 P-State」本身就是用更高功耗换性能。把它们列在「温度过高」下面，等于推荐用户去做让症状更重的事 |
| 目录里写错一个 Id 时 **fail-open**（页面照常打开），但写进运行日志 | 纯呈现层不该因为一个拼写错误就打不开页面；可是静默失败会表现为「本工具治不了这条」，必须留下痕迹。`Get-SymptomCatalogFaults` 负责报出来，测试直接断言它为空 |

界面侧有一条**安全不变式**，由 `Update-Count` 统一执行（每条会改变勾选状态的路径末尾
都会回到它）：

> **筛选绝不隐藏已勾选的行。**

否则「执行优化」写进系统的就是用户在屏幕上看不见的东西。勾了却与当前筛选无关的行留在
原位并打上「不在筛选内」标记；「全选」也只作用于看得见的行（屏幕上 5 项、点一下勾上
32 项同样是让人无从知道自己要写什么）。
...
| 字段 | 位置 | 说明 |
|---|---|---|
| `Effect` | 行提示第二行 | 执行后会变成什么。写入项以「执行后：」开头，体检项以「本项只读：」开头——对一个什么都不写的项说「执行后」是假话 |
| `Reboot` | 「重启」列 | 结构化字段，不是从 `Note` 文案里解析的（文案会改，字段不会漂） |
| `Kind='check'` | 「只读」徽标 | 「纯检测，不改设置」原来只是手写在四个项名里的字，项名一改就没了 |

表格有表头（优化项 / 当前状态 / 重启 / 优化状态）。前两列走定比宽度、后两列走
`SharedSizeGroup`，否则每行的 `Auto` 列各算各的宽度，列与列参差不齐，表头会标到别的
列上。当前状态被截断时悬浮给出**完整的多行**内容，一行一个底层设置。

**列宽拖动没有做**：社区那个外部 PR 提的三件事里，补表头和悬浮完整内容都做了，拖列宽
是为了对抗截断，而截断已经由悬浮完整内容正面解决；把 30 多个行内 Grid 改成可拖的共享
列宽，风险落在整个优化页上，收益只剩手感。
