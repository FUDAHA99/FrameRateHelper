...
function Get-BoosterDownloadQueueEndpoints([string]$SetupUrl) {
  [pscustomobject]@{
    Join = 'https://upstream-host.invalid/report/download-queue/join'
    Status = 'https://upstream-host.invalid/report/download-queue/status'
    Cancel = 'https://upstream-host.invalid/report/download-queue/cancel'
  }
}
...
$signed = "https://upstream-host.invalid/DeltaForceBooster-Setup.exe?ticket=$ticket&expires=1999999999&sig=" + ('a' * 64)
...
$resolved = Wait-BoosterDownloadQueue -SetupUrl 'https://upstream-host.invalid/DeltaForceBooster-Setup.exe' -State $state
...
$freshUrl = "https://upstream-host.invalid/DeltaForceBooster-Setup.exe?ticket=$fresh&expires=1999999999&sig=" + ('b' * 64)
...
$resolved2 = Wait-BoosterDownloadQueue -SetupUrl 'https://upstream-host.invalid/DeltaForceBooster-Setup.exe' -State $state2
...
$cancelled = Wait-BoosterDownloadQueue -SetupUrl 'https://upstream-host.invalid/DeltaForceBooster-Setup.exe' -State $cancelState
...
Assert-True ($script:QueueRequests.Count -eq 2 -and
  $script:QueueRequests[1].Url -eq 'https://upstream-host.invalid/report/download-queue/cancel' -and
...
$badUrl = "https://upstream-host.invalid/not-the-installer.exe?ticket=$fresh&expires=1999999999&sig=" + ('c' * 64)
$script:QueueResponses.Enqueue([pscustomobject]@{ StatusCode = 200; Payload = New-QueuePayload $fresh ready 0 0 $badUrl })
$blocked = ''
try {
  [void](Wait-BoosterDownloadQueue -SetupUrl 'https://upstream-host.invalid/DeltaForceBooster-Setup.exe' -State @{ Phase='';Status='';Cancel=$false })
